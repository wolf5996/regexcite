...
-   **2025-09-15:** End of chapter 11, which was a waste of time and energy—it was far too specific to be useful to a beginner or intermediate package developer
...
-   Match test files to R/ files: `R/foofy.R` → `tests/testthat/test-foofy.R`
...
-   Testing is harder than main code—this is normal
...
-   Tolerate duplication in test code—prioritize obviousness over *don't repeat yourself (DRY) principles*
...
-   Avoid `library()` calls in test files—they alter search path and create subtle bugs
...
**2025-10-28:** Stopped at section 14.3—cool section and good advice
...
-   **`tests/testthat/helper*.R`**: Files starting with ***"helper"*** execute before all tests. Perfect for custom test functions and setup code with side effects
    -   This is my preferred location for helper functions associated with testing. Putting them under `R/` seems like a very poor idea, and this location is definitely more natural—this very sentiment is echoed by the book
...
-   Do write descriptions when function purpose seems "obvious" to you—it won't be to users
...
-   Accept the duplication trade-off—easier for users, minimal extra maintenance
...
-   Mix inheritance with local overrides—document specialized args yourself, inherit the rest
...
#' # Also vectorised over pattern  ← highlights key feature
...
-   Vignette code may fail on CRAN/CI due to **credentials**, **slow runtime**, **API instability**, etc. → you must guard it.
...
-   Good practice: if code can’t run, print an **explanatory message** in a chunk with `eval = !can_run`.

-   Helpful chunk options:

    -   `include = FALSE` (run but don’t show)
    -   `echo = FALSE` (hide code)
    -   `error = TRUE` (allow errors without failing the whole vignette/check).

-   If you don’t want anything executed on CRAN: write a **pkgdown article** (website-only), not a vignette.

-   Workflow detail: `.Rmd` source lives in `vignettes/` and gets rendered at **`R CMD build`** into `inst/doc/`.

-   **`R CMD check` treats vignettes like tests**: it tangles and runs code, then rebuilds the vignette → code may execute **up to 2 times**, so failing chunks must be suppressed via `eval`.
...
-   Render with `devtools::build_readme()` — guarantees rendering against current source code
    -   This essentially updates the `readme.md` if the `readme.rmd` changes, which is ideal
-   Essential before release

**Automatic safeguards:**

-   Pre-commit Git hook added by `use_readme_rmd()` — blocks commits if README.Rmd is newer than README.md (override with `git commit --no-verify`)
...
-   `usethis::use_pkgdown()` — run once for initial setup
...
-   `pkgdown::build_site()` — run repeatedly to re-render the site locally into `docs/`
...
-   Moving through the rest of the book rapidly — another book on mathematics for bioinformatics is next; the goal now is awareness of available functionality, with deeper dives driven by need and application rather than theory

-   `usethis::use_pkgdown_github_pages()` handles the full setup in one call: creates an orphan `gh-pages` branch, enables GitHub Pages, configures a GitHub Actions workflow, and registers the site URL in DESCRIPTION and `_pkgdown.yml`

    -   It also calls `use_pkgdown()` automatically, so it's find to jump to this command straightaway

-   The site lives under `docs/` locally but has its own remote-only branch (`gh-pages`); GHA rebuilds the site on every push — conceptually very similar to a Netlify deployment workflow

-   Default URL pattern is `https://USERNAME.github.io/REPONAME/`

**Logo**

-   Hex sticker tradition — use `hexSticker` package to create, `usethis::use_logo()` to place it at `man/figures/logo.png`; pkgdown auto-discovers it

**Reference Index**

-   pkgdown renders all examples (not just source code) — review how they look on the site

    -   This is good to check on the site to ensure that figures look as expected and modify the dimensions of the output accordingly

-   Auto-linking works when functions are referenced in `[square brackets]` in roxygen comments; inbound links from other packages require the site URL in both DESCRIPTION `URL` field and `_pkgdown.yml`

-   For larger packages, organize the reference index into grouped sections via the `reference` field in `_pkgdown.yml`

**Vignettes & Articles**

-   Vignettes appear under the "Articles" dropdown; use `vignette("topic")` syntax for auto-linking within your package, `vignette("topic", package = "pkg")` for cross-package links

-   In vignettes/articles, reference your own functions with backticks and parentheses (`` `myfun()` ``); qualify external functions with namespace (`` `pkg::fun()` ``)

-   Non-vignette articles (`usethis::use_article()`) don't ship with the package bundle — useful when code requires informal dependencies, authentication, or produces large output that would hit CRAN size limits

-   Informal dependencies for articles go in `Config/Needs/website` in DESCRIPTION

**Development Mode**

-   Default `mode: release` builds a single site from current source (may document unreleased features)

-   `mode: auto` builds a main site from the released version and a `dev/` subdirectory from the development version — recommended for packages with a broad user base

**2026-02-04:** End of Chapter 19

-   Quick progress today because it is becoming more important to move on to more advanced topics

-   Starting next week, I will be working full-time on releasing the first version of my package

-   Tried building a site for this package

    -   Building the site worked but github pages hosting did not

-   Nvm, it worked quite well

    -   Just waiting on some GitHub action shit to run

# Chapter 20: Software Development Practices

Here's a concise summary:

**Software Development Practices for R Packages**

-   Use an IDE with R package development support (RStudio recommended, tightly integrated with devtools)
-   Adopt Git for version control — every R package should be a Git repo
-   Host repositories on GitHub to unlock the real benefits of version control (solo local Git alone rarely justifies the learning curve)
-   Set up CI/CD (continuous integration) to automatically run `R CMD check` and rebuild your pkgdown site on each push

**Why Git + GitHub Specifically**

-   Industry standard: 94% of developers use Git; 87% use GitHub for personal projects
-   R ecosystem alignment: \~80% of CRAN packages with URLs link to GitHub
-   Extensive learning resources due to popularity

**Key Benefits for R Packages**

-   **Communication**: GitHub Issues provide searchable, public bug reports and feature requests
-   **Collaboration**: Pull requests lower the barrier for outside contributions
-   **Distribution**: Others can install dev versions directly via `devtools::install_github()` or `pak::pak()`
-   **Website**: GitHub Pages integrates easily with pkgdown
-   **CI/CD**: Automate checking and deployment via GitHub Actions

**Core Argument**

-   These tools aren't overkill even for non-professional developers — they make it easier to experiment, document, test, and collaborate
-   Requires upfront investment but pays off by giving part-time developers access to expert-level workflows

**2026-02-10:** Stopped at section 20.2

-   I already use most or all of this, so this is a rehash of what I already know

**Summary: CI/CD with GitHub Actions**

-   **Core concept**: Host package on GitHub, configure automated tasks (like `R CMD check`) to run on events like pushes and pull requests

-   **Setup**: `usethis::use_github_action()` presents an interactive menu with pre-made workflows sourced from `r-lib/actions`

-   **Key workflows available**:

    -   `check-standard` — runs `R CMD check` on Linux, macOS, and Windows (highly recommended, especially for CRAN)
    -   `test-coverage` — computes test coverage via covr
    -   `pr-commands` — adds `/document` and `/style` commands for PRs
        -   There is also one for pkgdown that builds the site with every push

-   **What `use_github_action()` does**: writes a YAML workflow file to `.github/workflows/`, updates ignore files, and adds a status badge to your README

-   **Other GHA uses**: pkgdown site deployment to GitHub Pages, book publishing, or custom workflows using `r-lib/actions` building blocks

**2026-02-18:** End of Chapter 20

-   Short and sweet with some useful information around github actions, CI being continuous integration, and CD being continuous development

# Chapter 21: Lifecycle

-   Package evolution is about balancing the needs of maintainers, existing users, and future users — you can't optimize for all simultaneously

-   The smallest meaningful unit of change is a **Git commit**, but the primary signal of meaningful change to users is an **incremented version number** paired with a release

-   Version numbers live in the `Version` field of `DESCRIPTION` and each CRAN release creates an archived package bundle

-   **Breaking changes** (removing functions, narrowing inputs) are the main source of user friction — later sections cover how to gauge "breakingness" and when breaking changes are justified

-   Key practices for managing change kindly: intentional **version numbering** (patch vs major), explicit **lifecycle stages** (experimental, superseded, deprecated, stable), and phased **deprecation processes**

-   The unofficial **`cran/` GitHub mirror** (e.g., `github.com/cran/usethis`) is extremely useful for quickly comparing CRAN releases — each commit corresponds to a release, making diffs trivial compared to navigating the actual dev repo or downloading `.tar.gz` archives from CRAN

-   Three complementary views of a package's evolution: CRAN archive (official bundles), CRAN GitHub mirror (easy diffing), and the dev repo (full commit history)

**2026-02-23:** Stopped at section 21.2

-   Brisk progress and a lot of this is making sense now

**Tidyverse Version Convention: `<major>.<minor>.<patch>`**

-   Always use `.` as separator, never `-`

-   Always spell out all three components — use `1.0.0`, never `1.0`

-   **Major**: big, potentially breaking changes (users may need to update their code)

-   **Minor**: new features that are backward-compatible

-   **Patch**: bug fixes, minor improvements, no new features

    -   All of the changes I make to arguments, defaults, etc. are considered patches, new functions entirely are minor changes, and breaking changes to the code are considered major

**Backward Compatibility & Breaking Changes**

-   A **breaking change** means pre-existing code no longer works (or works differently) with the new version — though "works" is a spectrum, not binary

-   Assess impact from three angles: severity of behavior change, how central the affected code is to the package, and how much existing usage is affected

-   **Typically breaking**: removing a function, removing an argument, narrowing valid inputs

-   **Typically not breaking**: adding a function, adding an argument, expanding valid inputs, fixing a bug, changing error/print text — though each has edge-case caveats

-   Pragmatic definition: a change is breaking if it causes a previously-passing reverse dependency to fail `R CMD check` and the dependency's usage was correct

-   Best way to assess impact is empirical — run your own tests plus reverse dependency checks, don't just reason about it

**2026-02-24:** Stopped at section 21.5

-   Brisk pace as always because I need to replace this slot in my to-do list with the deep learning and python work

-   Some really good points here though

Here's a summary of the key points:

**Version Number Structure**

-   Format: `major.minor.patch` for releases, with `.dev` appended during development
    -   I am usually sticking with the first part of this and ignoring the second part—never got sold on the `.dev` convention
-   **Patch release** (e.g., 0.8.2): bug fixes only, no new features or breaking changes
    -   Changing default values for a function is a patch release
-   **Minor release** (e.g., 0.9.0): bug fixes, new features, and backward-compatible changes — the most common release type
    -   Add new functions or arguments to an existing function is a minor release
-   **Major release** (e.g., 1.0.0): breaking changes that affect many users; 1.0.0 specifically signals a feature-complete, stable API
    -   Massive changes to the API that break compatibility with the last version of the package is a major release
    -   Alternatively, it should be a bunch of new cool features that widen the scope of your work
-   Save exciting new features for major releases for better visibility (blog posts, NEWS)

**The `usethis` Utility for Versioning**

-   `usethis::create_package()` initializes your package at version `0.0.0.9000`
-   `usethis::use_version()` provides an interactive menu to increment major, minor, patch, or dev versions — no manual editing needed
-   It automatically updates `Version` in DESCRIPTION and adds a new heading in `NEWS.md`, keeping your changelog in sync with your version bumps
    -   You can then have Claude Code tag the specific commit on GitHub and build the release afterwards

**Breaking Changes: Practical Considerations**

-   Most R users don't manage package versions intentionally, so upgrading to a major version with breaking changes often happens accidentally
-   The `renv` package is recommended for project-specific libraries to protect against unexpected breakage
-   Extreme resistance to breaking change drags down development, but constant breaking changes frustrate users—aim for a balanced middle ground
-   The more users you have, the more carefully you should weigh backward compatibility against development velocity

**2026-03-06:** Stopped at section 21.7

-   Brisk pace and valid points above with a special shout-out to the cool `usethis::use_version()` function that I just learnt about

**Lifecycle Stages and Tools**

-   The lifecycle package (lifecycle.r-lib.org) provides tooling for managing package evolution
-   Two major components: lifecycle stages (applied to arguments, functions, or packages) and conventions/functions for transitioning between stages

**Four Lifecycle Stages**

-   **Stable** — the default; users can rely on it; breaking changes should be rare and gradual
-   **Experimental** — function may change without much deprecation process; implied for any package with major version 0 (pre-1.0.0)
-   **Deprecated** — slated for removal; still works initially but triggers a warning with info on alternatives; eventually removed
-   **Superseded** — a softer alternative to deprecated; legacy functionality preserved in a "time capsule" with only critical bug fixes

**Badges**

-   `usethis::use_lifecycle()` does one-time setup: adds lifecycle to Imports, copies SVG badges to `man/figures/`, sets up the namespace
-   For functions, include the badge in the `@description` block using inline R: `` `r lifecycle::badge("superseded")` ``
-   For arguments, include the badge in the `@param` tag
-   `usethis::use_lifecycle_badge()` adds a package-level badge to README
-   No badge needed if the stage matches the parent level (e.g. a stable function in a stable package)

**Deprecating a Function**

-   Use `lifecycle::deprecate_warn()` inside the function body to inform users and point them to the preferred alternative
-   The warning message is built from structured inputs (`when`, `what`, `with`, `details`) for a predictable, low-cognitive-load format
-   By default, each specific warning fires only once every 8 hours to balance annoyance with actionability; set `always = TRUE` near end of life
-   `lifecycle::deprecate_soft()` only warns the person who can actually fix it — indirect callers (e.g. users of a downstream package) are not warned by default

**Typical Deprecation Schedule**

-   **v1.5.0** — `fun()` exists and is stable
-   **v1.6.0** — deprecation begins: add a deprecated badge to the help topic, insert `deprecate_warn()` in the function body; `fun()` still works
-   **v1.7.0 or v2.0.0** — `fun()` is removed; whether minor or major release depends on how widely used it is

**Deprecating an Argument**

-   Use `lifecycle::deprecated()` as the default value for the deprecated argument — signals deprecation in docs and enables `lifecycle::is_present()` checks inside the function
-   Inside the function, check `is_present(old_arg)`, issue `deprecate_warn()`, then map the old argument to the new one so existing code still works
-   Base R alternative: `missing()` overlaps with `is_present()` but is trickier around default values; `.Deprecated()` and `.Defunct()` substitute for `deprecate_warn()`

**Deprecation Helpers**

-   When a deprecation affects many functions (e.g. removing a `verbose` argument from every function), centralize the logic in an internal helper rather than inlining it everywhere

-   The googledrive example: `verbose` argument was replaced by a global option (`googledrive_quiet`) and scoped helpers (`local_drive_quiet()`, `with_drive_quiet()`)

-   Pattern: each affected function uses `verbose = deprecated()` as default, then delegates to the shared helper `warn_for_verbose()`

-   The helper checks `is_present(verbose)` — if unspecified or `TRUE`, it does nothing; if `FALSE`, it warns with advice on the new approach while still honoring the user's intent for now

-   Once the next major release arrives, the argument and helper can both be deleted cleanly

**Dealing with Change in a Dependency**

-   If the dependency is already released, you can bump the minimum version in DESCRIPTION and use the new feature unconditionally — but this forces users to update that dependency too (and only works for Imports, not Suggests)

-   To support both old and new versions, check `packageVersion("otherpkg")` at runtime and branch accordingly; `rlang::is_installed()` and `rlang::check_installed()` with the `version` argument are good alternatives

-   For not-yet-released changes in a dependency, you can prepare a version of your package that works before and after the change — sometimes via refactoring, sometimes via explicit version checks against a dev version like `1.3.9000`

-   If referencing a function that only exists in the dev version, `R CMD check` will note it doesn't exist yet; CRAN generally accepts this if you explain it's for backward/forward compatibility

**Superseding a Function**

-   Superseded is appropriate when a function is no longer preferred but has enough usage that removal isn't warranted — e.g. `tidyr::spread()` / `tidyr::gather()` superseded by `pivot_wider()` / `pivot_longer()`

-   Superseded functions receive only critical bug fixes and no new features, but are not at risk of removal

-   A related pattern is preserving legacy behaviour behind an opt-in mechanism so users can get old code working quickly until they can do a full update

-   Examples of opt-in legacy behaviour: `tidyr::nest_legacy()` / `unnest_legacy()` for the old nest interface; `dplyr.legacy_locale` option for pre-1.1.0 group sorting; `vctrs::vec_as_names_legacy()` for older name-repair strategies; `readr::with_edition(1)` / `local_edition(1)` for the pre-2.0.0 backend

**2026-03-09:** End of chapter 21

-   Applied `usethis::use_lifecycle()` to my own package

    -   I intend to mark that as a patch as well (**v1.3.1**) in line with package dev conventions discussed in this chapter
...
-   `usethis::use_release_issue()` opens a GitHub issue with a release checklist
    ...
    -   It's responsive to package characteristics and constantly evolving

-   Treat CRAN compliance as an ongoing practice, not a last-minute activity

    -   Run `R CMD check` regularly on multiple platforms and fix issues promptly

-   Main reason to publish on CRAN: discoverability and reach

    -   most R users only install from CRAN

-   Even a clean local `R CMD check` can fail on remote servers with different OS/configurations — CI via GitHub Actions is essential

    -   I already do this for macOS, Ubuntu, and windows through GitHub Actions for my package `BadranSeq`

**Release process overview**

-   Choose release type (major/minor/patch), which determines version number and shapes the checklist

-   For existing CRAN packages: review current CRAN check results; for new packages: confirm CRAN policy compliance

-   Freshen `README.md` and `NEWS.md`

-   Run `devtools::check()` across multiple OS and R versions (release + devel)

-   Run reverse dependency checks if other packages depend on yours

-   Submit to CRAN, await acceptance, then create a GitHub release and bump the version number

**First release considerations**

-   New packages get human review in addition to automated checks

    -   higher scrutiny and some subjectivity

-   `usethis::use_cran_comments()` creates a file for submission comments (later used for reporting revdep results)

-   Title and Description in `DESCRIPTION` are heavily nitpicked

    -   Proofread carefully

-   `Authors@R` must include a copyright holder (`'cph'` role)

    -   Either the author or employer

-   Maintainer email must be stable and unfiltered

    -   CRAN will remove packages if they can't reach you

-   Every exported function needs `@returns` and `@examples` — don't use `\dontrun{}` as a shortcut; express pre-conditions properly

-   Embedded third-party code must have its license correctly declared

-   Check community lists of recent CRAN "gotchas" (e.g., [`extrachecks`](https://github.com/DavisVaughan/extrachecks) repo) to pre-empt common rejection reasons

**Staying informed on CRAN policy**

-   Official policy lives at `cran.r-project.org/web/packages/policies.html`

-   The `eddelbuettel/crp` GitHub repo tracks policy changes via commit history — much easier to monitor than re-reading the document

-   R-package-devel mailing list is useful for keeping up with what other maintainers encounter

**2026-03-12:** Stopped at section 22.3

-   Last chapter of the book and will guide my efforts if I ever want to submit this to CRAN

-   Might be worth doing after submitting this for publication through bioArchives or JOSS

**Keeping Up with Change**

-   Check current CRAN check results for your package — address any non-"OK" results before releasing

-   Advance any ongoing deprecation processes (see lifecycle/versioning notes)

-   Polish accumulated `NEWS` bullets for clarity and consistency

-   Run `urlchecker::url_check()` — fix dead URLs and handle 301 redirects (convert to verbatim text if a legitimate URL triggers CRAN's redirect policy)

-   Rebuild `README.md` with `devtools::build_readme()` to ensure it reflects current package state

**Double R CMD Checking**

-   `devtools::check(remote = TRUE, manual = TRUE)` — full check on your dev machine

-   `devtools::check_win_devel()` — check against r-devel on CRAN's win-builder; required by CRAN policy

-   CRAN expects packages to work on at least two major platforms; packages failing this are typically rejected

-   **ERRORs and WARNINGs**: must all be fixed — CRAN will not accept otherwise

-   **NOTEs**: eliminate as many as possible; any you can't fix must be explained in `cran-comments.md`

-   First submissions always get one unavoidable NOTE — just acknowledge it

**CRAN Check Flavours and Services**

-   CRAN checks across combinations of OS (Windows, macOS, Linux), R version (devel, release, oldrel), compilers, and locale

-   Use GitHub Actions as primary multi-platform CI; supplement with R-hub (`rhub::check_for_cran()`) and the macOS builder (`mac.r-project.org`) for additional coverage

**Reverse Dependency Checks**

-   Run `revdepcheck::revdep_check(num_workers = 4)` — checks each revdep twice (CRAN version vs your dev version) to isolate new breakages from pre-existing failures

-   One-time setup: `usethis::use_revdep()` configures `.gitignore` and `.Rbuildignore`

-   Results written to `revdep/` — `README.md` (maintainer summary), `problems.md` (breakages), `failures.md` (unchecked), `cran.md` (paste into `cran-comments.md`)

-   Be aware of false positives (flaky tests, resource limits) and false negatives (missing system deps preventing installation)

-   If your release breaks other packages: notify maintainers (ideally with a patch/PR), give at least two weeks' notice before submission, explain remaining failures in `cran-comments.md`

**Update Comments for CRAN**

-   `cran-comments.md` — version-controlled, `.Rbuildignore`d file recording R CMD check results and revdep summary

-   Structure: "R CMD check results" section (errors/warnings/notes) + "revdepcheck results" section (or state no downstream deps)

-   Explain any NOTEs with the check message and justification

**The Submission Process**

-   `usethis::use_version('minor')` (or `'patch'`/`'major'`) — bump version in DESCRIPTION

-   `devtools::submit_cran()` — builds package, uploads to CRAN, writes a local `CRAN-SUBMISSION` file used later by `use_github_release()`

-   You must click the confirmation link in the email CRAN sends — without this your package is not actually submitted

-   CRAN runs automated checks (Windows + Linux, release + devel); initial submissions also get human review

**Failure Modes**

-   Three ways to fail: R CMD check failure (automated), policy violation (human review), or revdep regressions (automated)

-   On failure: fix problems, bump patch version (gaps in version numbers are fine), add a "Resubmission" section to `cran-comments.md` listing changes, then `devtools::submit_cran()` again

-   If the failure is a false positive (common with revdep checks on large packages), reply to CRAN's email with a concise explanation

**Celebrating Success**

-   `usethis::use_github_release()` — pushes the release tag and creates GitHub release notes from `NEWS`; depends on the `CRAN-SUBMISSION` file

-   `usethis::use_dev_version(push = TRUE)` — immediately increment to dev version so new branches are clearly based on development

-   Check your CRAN landing page a few days post-release — binaries and full flavour checks take time and can surface new issues

-   Blog about the release (especially for initial/minor/major); link blog posts in `_pkgdown.yml` under the news menu

-   Future releases should be spaced at least 1–2 months apart per CRAN policy

**2026-03-16:** End of chapter 22 and the book

-   Was a very long ride that resulted in the development of [BadranSeq](https://github.com/wolf5996/BadranSeq)
